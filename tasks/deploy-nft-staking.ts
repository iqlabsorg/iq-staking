...
.addParam('owner', 'New owner address', undefined, types.string, false)
.setAction(async ({ proofSource, stakingManager, nftCollectionAddress, owner }, hre) => {
  const [deployer] = await hre.ethers.getSigners();

  console.log('Deploying...', { proofSource, stakingManager, nftCollectionAddress, owner });
  ...
  const { address, transactionHash } = await hre.deployments.deploy('IQNftStaking', {
    ...
    args: [proofSource, stakingManager, nftCollectionAddress, owner],
  });
  console.log('IQNftStaking deploy tx:', transactionHash);
  console.log('IQNftStaking address:', address);

  await verifyContract(hre, address, [proofSource, stakingManager, nftCollectionAddress, owner]);
